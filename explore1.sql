...
# First, I will use a CTE to rank each player by OPS, partitioned by the team they played for in 2021.
...
# In the outer query, I pull each team's top OPS player, then use a series of subqueries to pull up the 2nd and 3rd place players' names and OPS, so that
# it appears in one row.
SELECT
    ops.Team,
...
# I want to see how ballpark dimensions and wall height might affect home team offensive performance. 
# To do this I will create a new view that includes each ballpark's dimensions and wall height and their ranking among other ballparks.
...
#Each ballpark's depth and height rankings, and sum of homeruns hit by the hometeam in 2021:
...
#Using dimension rankings view and home away batting splits, I looked at average home runs per season hit by the home team grouped by wall height and depth. 
#Used a rollup function to examine the differences when accounting for both wall size and depth, as well as each individually.
SELECT 
...
#The rollup output was a bit messy, so here is a cleaned up version with fewer groupings:
...
# To dive deeper into Giants monthly stats, I will create a view of Giants batting by month
